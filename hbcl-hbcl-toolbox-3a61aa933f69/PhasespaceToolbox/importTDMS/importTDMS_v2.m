function dataStruct=importTDMS_v2(TDMSfilename,SaveConvertedFile,DisplayContents)
% 
...
% Editor: Lanxin Liu
%         Human Biomechanics and Control Laboratory, University of Michigan
%         lanxliu@umich.edu
%         734-604-0006
% Last Modified: 5/24/2011
% Modifications: Saves a file automatically even when run without inputs
%                Asks for option to store time data from excel file
...
%Asks to see to include time data or not
checkTime = input('Would you like to include start times in the data? Y/N\nNote: you must have the excel version of the file in order to do this: ', 's');

%Take time data into .mat file from excel file
if checkTime == 'Y'
    [xlsfile,xlsfolder]=uigetfile('*xlsx', 'Choose the excel file');
    [num, txt, raw] = xlsread(strcat(xlsfolder, xlsfile));
    for i = 1:numgrps
        time = txt(8 + numgrps + (numchans + 3) * (i - 1), 22);
        for j = 1:numchans
            dataStruct.group(i).channel(j).property(11).value = char(time);
        end
    end
end


%Slight change made to save automatically
cd(filefolder);
saveName = strcat(dataStruct.property(1).value, '_data');
save(saveName, '-struct', 'dataStruct');
