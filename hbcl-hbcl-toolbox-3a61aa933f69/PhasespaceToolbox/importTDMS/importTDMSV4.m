function dataStruct = importTDMSV4(tdmsFilename)
%
...
switch nargin
  case 0
    ...
    [filepath,filefolder]=uigetfile({'*.tdm;*.tdms'}, 'Select a TDM or TDMS file', 'MultiSelect', 'off');
    if isempty(filepath),
      return;
    end
    %     if ~iscell(filepath),
%     tdmsFilename = cellstr(fullfile(filefolder,filepath));
  ...
  case 1
    if ~ischar(tdmsFilename) % && ~iscell(TDMSfilename),
      error('first argument must be a string representation of a tdms file');
    end
    
  otherwise
    error('too many arguments to importTDMSV3');
    return
    
end

%% Process each file
Data_Path = tdmsFilename;
...
%             [err,dummyVar,year,month,day,hour,minute,second,msecond,wkday]=calllib(libname,'DDC_GetChannelGroupPropertyTimestampComponents',chanHandles(j),channelPropNames{i}{j}{k+1},yearIn,monthIn,dayIn,hourIn,minuteIn,secondIn,msecondIn,wkdayIn);
            [err,dummyVar,year,month,day,hour,minute,second,msecond,wkday]=calllib(libname,'DDC_GetChannelPropertyTimestampComponents',chanHandles(j),channelPropNames{i}{j}{k+1},yearIn,monthIn,dayIn,hourIn,minuteIn,secondIn,msecondIn,wkdayIn);
      ...
      switch channelDataType{i}{j}
        ...
        case 'DDC_String'
          ...
          dataValues = cell(1,numVals);
          for tmp = 1:double(numVals)
            dataValues{tmp}=blanks(maxNumberOfCharacters);
          end
...
% Post process the data into a nice MATLAB structure
dataStruct = postProcess( ...
  Data_Path, topLevelFilePropNames, topLevelFilePropValues, groupPropNames, ...
  groupPropValues, channelPropNames, channelPropValues, channelDataValues);
...
if err==0,
  ...
  for i=1:numel(month)
    ...
    retStr{i} = [num2str(day(i)) ' ' monthTxt ' ' num2str(year(i)) ', ' num2str(hour(i)) ':' num2str(minute(i)) ':' num2str(double(second(i))+msecond(i)/1000,'%6.3f')];
  end
