...
%%%%%%% Peter Adamczyk 2007 March 20:: average instead of resample 
sz = size(signals);
if iscell(signals)
 truncated = num2cell( squeeze( mean( cell2mat(reshape( signals,[mult,index2-index1+1,sz(2:end)]) )' ,2) ) );
else
 truncated = squeeze( mean( reshape( signals,[mult,index2-index1+1,sz(2:end)])' ,2) );
end
%--------------------------------------------------------------------------
