...
% 'treadmillDataFromMatlab'
%   Default is true. Set to 0 to process TDMS files. Not actively supported.
% 'tdmsTabNumber'
...
%   the custom phasespace log have the same marker names (or tracked is the
...
% Minor updated 2014.08.05 by Amy Wu for marker renaming
...
if (~isempty(c3dFilenameToLoad))
  ...
  markerNames = trackedC3dTrial.markerNames(1,:);
  numberOfMarkers = length(markerNames);
...
%     if (abs(maxLag) > 1000)
%       error('the lag between the c3d marker data and the markerDataCustomLog is large, and the resulting curves don''t match well. Are you sure you are specifying c3d and customLogs that correspond to each other?');
%     end
%   end
  ...
  for markerNumber = 1 : length(markerNames)
    ...
    try
      ...
      % never write a comment like the following. Where, for example, might
      % one find "his plots"? Does he intend to have these available for
      % the next few decades anytime anyone wants to see them? And if
      % someone comes up to him and asks him for "his plots", would he know
      % what to show them?
...
% description = '';
% lockParam = '0';
% dataType = int16(2); %2); %4; %-1; %-1 : Character, 1 : Byte, 2 : Integer, 4 : Floating point
...
%   dataDimensions, data); %[]); %
% 
% parameterName = 'ACTUAL_START_FIELD';
% data = int16([1, 0]);
% 
% c3dServer.AddParameter(parameterName, description, ...
%   parameterGroup, lockParam, dataType, numberOfDataDimensions, ...
...
% % c3dServer.SetParameterType(nIndex, 2);
% % analogIndex = 0;
...
% data = numberOfFrames; %int16([mod(numberOfFrames, (2^16 - 1)), floor(numberOfFrames / (2^16 - 1))]);
% 
% c3dServer.AddParameter(parameterName, description, ...
%   parameterGroup, lockParam, dataType, numberOfDataDimensions, ...
%   dataDimensions, data); %[]); %
...
for markerNumber = 1:numberOfMarkers
  ...
  % this is how it used to be, but we probably instead want to use the data
  % from the c3d?
