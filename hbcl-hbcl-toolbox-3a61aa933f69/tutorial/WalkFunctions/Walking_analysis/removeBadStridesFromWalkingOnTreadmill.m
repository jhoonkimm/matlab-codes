...
badStrideTimeThreshold = 0.50; % percent diff from mean (e.g., 0.1 = 10%)
badStepTimeThreshold = 0.50;
...
for i=1:numRsteps-1
    ...
    if i~=1
    index_step(:,i-1)=[diff([loadingSlopeLGRFzl(i) medianLoadingSlopeLGRFzl]) > slopeThreshold*medianLoadingSlopeLGRFzl ;...
    diff([loadingSlopeRGRFzr(i) medianLoadingSlopeRGRFzr]) > slopeThreshold*medianLoadingSlopeRGRFzr;  ...
            diff([unloadingSlopeLGRFzl(i-1) medianUnloadingSlopeLGRFzl]) < slopeThreshold*medianUnloadingSlopeLGRFzl  ;... 
            diff([unloadingSlopeRGRFzr(i) medianUnloadingSlopeRGRFzr]) < slopeThreshold*medianUnloadingSlopeRGRFzr; ... 
            abs((Rstridetime(i)-avgstridetime)) > badStrideTimeThreshold*mean(Rstridetime);  ... 
            abs((Rsteptime(i)-avgRsteptime)) > badStepTimeThreshold*mean(Rsteptime);  ... 
            abs((Lsteptime(i-1)-avgLsteptime)) > badStepTimeThreshold*mean(Lsteptime) ; ... 
            abs((RswingPhaseTimeR(i)-median(RswingPhaseTimeR))) > badStrideTimeThreshold*median(RswingPhaseTimeR) ; ... 
            abs((LswingPhaseTimeR(i)-median(LswingPhaseTimeR))) > badStrideTimeThreshold*median(LswingPhaseTimeR) ];
    end
