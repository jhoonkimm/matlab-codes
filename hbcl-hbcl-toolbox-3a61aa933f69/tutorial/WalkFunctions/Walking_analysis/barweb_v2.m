function handles = barweb(barXvalues,barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot bars
	handles.bars = bar(barXvalues,barvalues, width,'edgecolor','k', 'linewidth', 2);
	hold on
