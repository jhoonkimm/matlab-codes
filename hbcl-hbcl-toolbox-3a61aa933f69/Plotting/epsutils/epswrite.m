...
%                    Applies all the fixes if 'on'. If turned 'off', some
%                    of the enhancement function may not be usable on the
%                    generated EPS file. If HG2 is enabled, this option is
%                    forced to 'off'.
%
...
% Copyright 2012-2013 Takeshi Ikuma
% History:
...
%          - Bug fix
% rev. 8 : (05-20-2013)
%          - Bug fix. Make sure only figure children objects with 'Units' 
%            property are chosen as h.
%          - Added 'Postprocessing' option to turn off fixes
% rev. 9 : (08-05-2013)
%          - Checks HG2 feature and disable postprocessing if HG2 is
%            enabled
...
axpnames = {'Units'};
h = findobj(get(H,'Children'),'flat','-property','Units');
...
% done if no postprocessing requested
if ~params.postprocessing, return; end
...
% turn off postprocessing if HG2 is enabled
try % try-catch block for older release, in which UseHG2 is not a valid feature
   params.postprocessing = params.postprocessing && ~feature('UseHG2');
catch
end
