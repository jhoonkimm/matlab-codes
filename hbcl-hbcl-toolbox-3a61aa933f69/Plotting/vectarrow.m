...
%   $Revision: 1.0
% Karl Zelik modified to include color option (October 2009)
...
if nargin > 2
  ...
  if max(size(p0))==3
      if max(size(p1))==3
          ...
          y1 = p1(2);
          z1 = p1(3);
          plot3([x0;x1],[y0;y1],[z0;z1],'Color',linecolor);   % Draw a line between p0 and p1
          ...
          hold on
          plot3(hu(:),hv(:),hw(:),'Color',linecolor)  % Plot arrow head
          grid on
          xlabel('x')
          ylabel('y')
          zlabel('z')
  ...
  elseif max(size(p0))==2
      if max(size(p1))==2
          ...
          x1 = p1(1);
          y1 = p1(2);
          plot([x0;x1],[y0;y1],'Color',linecolor);   % Draw a line between p0 and p1
          ...
          hold on
          plot(hu(:),hv(:),'Color',linecolor)  % Plot arrow head
          %grid on
          xlabel('x')
          ylabel('y')
          hold off
