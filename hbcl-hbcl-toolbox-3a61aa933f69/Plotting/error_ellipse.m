function [ellipseToDraw, handleToEllipse] = error_ellipse(varargin)
...
C = prop.C;
useFill = prop.useFill;
plotOutline = prop.plotOutline;
...
elseif r==2 & c==2
  ...
  ellipseToDraw = [scale*(x0+k*x) scale*(y0+k*y)];
  
  if (useFill)
    handleToEllipse = fill(scale*(x0+k*x), scale*(y0+k*y), prop.fillStyle{:});
    %     h1 = fill(scale*(x0+k*x), scale*(y0+k*y), 'g');
  elseif (plotOutline)
    %     h1 = plot(scale*(x0+k*x),scale*(y0+k*y),prop.style{:});
    if (ischar(prop.style))
      handleToEllipse = plot(scale*(x0+k*x),scale*(y0+k*y),prop.style);
    else
      handleToEllipse = plot(scale*(x0+k*x),scale*(y0+k*y),prop.style{:});
    end
  end
