function p = repAnova1(x,plotson)
...
if(e<0.75)
  ...
  eps=e;
elseif(e < 1 - 1e-10)				% epsilon correction of Huynh and Feldt  - corrected 1/15/2008 by Steve Collins stevehcollins@gmail.com
  eTilde=(n*(p-1)*e-2)/((p-1)*(n-1-(p-1)*e));
  if(eTilde<1)
    corr=1;
    eps=eTilde;
    df_treat=floor(eTilde*df_treat);
    if df_treat == 0;  fprintf(1,'warning: df_treat failure in repAnova1 \n'); end
    df_res=floor(eTilde*df_res);
    if df_res == 0;  fprintf(1,'warning: df_res failure in repAnova1 \n'); end
  end
end
