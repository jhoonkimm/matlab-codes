function [RHS,LHS,RTO,LTO] = findEventIndicesForWalkingOnTreadmill(Rgrf, Lgrf, varargin)
...
%  Uses flags (see below). Example: 
%  findEventIndicesForWalkingOnTreadmill(RGRF, LGRF, 'dataFrequency',960,'showPlots',1)
% 
...
%  Flags:
%  'dataFrequency'
%    Default is 120. Sampling frequency of grf in Hz. 960 as of March 2014.
%  'showPlots'
%    Default is false. Show the debug plot.
%  'figureNum'
%    Default is 25000. Figure number for debug plot.
%  'zForceThreshold'
%    Default is 0.3. mean(Fz)*zForceThreshold = Fz threshold used in
%    algorithm.
...
%% Quasi-constants and initializiations
samplingfreq = 120;
...
while ~complete
    ...
    jStartInd=max(2,max([find(nearRHS-windowSize>0,1), find(nearLHS-windowSize>0,1), find(nearRTO-windowSize>0,1),find(nearLTO-windowSize>0,1)]));
    
    for j=jStartInd:max(find(nearRHS+windowSize<length(Rgrf))) % length(nearRHS)-1
        for i = nearRHS(j):-1:nearRHS(j)-windowSize
            if Rgrf(i,3)==0 | Rgrf(i,3)<Rgrf(i-n,3) | i==nearRHS(j)-windowSize % find closest zero, local minimum or end of window
                RHS = [RHS; i];
                break;
            end
        end
    ...
    for j=jStartInd:max(find(nearLHS+windowSize<length(Rgrf))) % length(nearLHS)-1
        for i = nearLHS(j):-1:nearLHS(j)-windowSize
            if Lgrf(i,3)==0 | Lgrf(i,3)<Lgrf(i-n,3) | i==nearLHS(j)-windowSize
                LHS = [LHS; i];
                break;
            end
        end
    ...
    for j=jStartInd:max(find(nearRTO+windowSize<length(Rgrf))) % length(nearRTO)-1
        for i = nearRTO(j):1:nearRTO(j)+windowSize
            if Rgrf(i,3)==0 | Rgrf(i,3)<Rgrf(i+n,3) | i==nearRTO(j)+windowSize
                RTO = [RTO; i];
                break;
            end
        end
    ...
    for j=jStartInd:max(find(nearLTO+windowSize<length(Rgrf))) % length(nearLTO)-1
        for i = nearLTO(j):1:nearLTO(j)+windowSize
            if Lgrf(i,3)==0 | Lgrf(i,3)<Lgrf(i+n,3) | i==nearLTO(j)+windowSize
                LTO = [LTO; i];
                break;
            end
        end
